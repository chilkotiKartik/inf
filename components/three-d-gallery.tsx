...
// 3D Model Components
function SpacecraftModel({ autoRotate }: { autoRotate: boolean }) {
	const meshRef = useRef<any>()
	useFrame((_, delta) => {
		if (autoRotate && meshRef.current) meshRef.current.rotation.y += delta * 0.2
	})
	return (
		<group ref={meshRef}>
			<mesh position={[0, 0, 0]}>
...
function DroneModel({ autoRotate }: { autoRotate: boolean }) {
	const meshRef = useRef<any>()
	const propellerRefs = [useRef<any>(), useRef<any>(), useRef<any>(), useRef<any>()]
	useFrame((_, delta) => {
		if (autoRotate && meshRef.current) meshRef.current.rotation.y += delta * 0.2
	...
	return (
		<group ref={meshRef}>
			...
			{[0.75, -0.75, 0.75, -0.75].map((x, i) => (
				<group key={i} position={[x, 0, i < 2 ? 0.75 : -0.75]}>
					<mesh>
						<cylinderGeometry args={[0.05, 0.05, 0.1, 8]} />
						<meshStandardMaterial color="#1f2937" metalness={0.7} roughness={0.2} />
					</mesh>
					<mesh ref={propellerRefs[i]} position={[0, 0.05, 0]}>
...
function SatelliteModel({ autoRotate }: { autoRotate: boolean }) {
	const meshRef = useRef<any>()
	useFrame((_, delta) => {
		if (autoRotate && meshRef.current) meshRef.current.rotation.y += delta * 0.2
	})
...
export default function ThreeDGallery() {
	...
	const filteredModels = useMemo(
		() => models.filter((model) => activeTab === "all" || model.category === activeTab),
		[activeTab]
	)
	...
	return (
		<div className="relative">
			...
			<div className="container mx-auto px-4 py-6">
				<Tabs defaultValue="spacecraft" className="mb-6" onValueChange={setActiveTab}>
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
					...
					<Card className="bg-card/50 backdrop-blur-sm border-border lg:col-span-2">
						...
						<CardContent>
							<div ref={canvasRef} className="h-[350px] w-full relative">
								{isLoading ? (
