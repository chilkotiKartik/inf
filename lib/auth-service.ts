...
export const signIn = async (email: string, password: string) => {
  try {
    // Try mock authentication first for faster login
    if (mockAuthEnabled) {
    ...
    try {
      ...
      throw new Error("Invalid credentials. Please check your email/roll number and password.")
    }
  } catch (error: any) {
...
export const signOutUser = async () => {
  try {
    ...
    // Try Firebase sign out (won't throw error if no Firebase user)
    try {
      await signOut(auth)
    } catch (firebaseError) {
      // Firebase signout failed, but that's okay for mock users
      console.warn("Firebase signout failed (expected for mock users):", firebaseError)
    }
  } catch (error: any) {
